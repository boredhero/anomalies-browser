...
def setup_logging(level: str = "INFO", log_dir: str | None = None) -> None:
    ...
    Log files go to log_dir/hole_finder.log (rotated daily in production
    ...
    # File handler (if log_dir specified or /data/hole-finder/logs exists)
    if log_dir is None:
        for candidate in ["/data/hole-finder/logs", "/app/logs"]:
    ...
    if log_dir:
        ...
        file_handler = logging.FileHandler(log_path / "hole_finder.log")
